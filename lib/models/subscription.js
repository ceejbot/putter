...
var Subscription = polyclay.Model.buildClass(
{
    properties:
    {
...
polyclay.persist(Subscription, '_id');
...
Subscription.removeByOwner = function(owner, callback)
{
    ...
    {
        return function(callback) { Subscription.adapter.db.remove(id, rev, callback); };
    };

    Subscription.findIDsByOwner(owner, function(err, structs)
    {
        if (err) return callback(err);
