...
function Searcher(config)
{
	...
	this.indexname = config.index || '/putter';
...
{
	bhttp.get(this.urlbase, function(err, response, data)
	{
		callback(err, data);
	});
};
...
Searcher.prototype.registerDocumentType = function(modelClass, callback)
{
	...
	bhttp.put(this.urlbase, '', function(err, response, data)
	{
		// now create a mapping for our data type
		var mappath = path.join(self.indexname, type, '_mapping');
		var opts = { encodeJson: true };
		bhttp.put(self.urlbase + '/' + type + '/_mapping', mapping, opts, function(err, response, body)
		{
...
Searcher.prototype.dropDocumentType = function(modelClass, callback)
{
	...
	bhttp.delete(this.urlbase, opts, function(err, response, body)
	{
		if (body && body.error) err = body.error;
		callback(err, body);
	});
...
Searcher.prototype.index = function(document, callback)
{
	...
	var indexpath = path.join(document.plural, data.id);
	if (self.flushImmediately)
		indexpath += '?refresh=true';

	var opts = { encodeJson: true };
	var url = this.urlbase + '/' + indexpath;

	bhttp.put(url, data, opts, function(err, response, body)
	{
		if (body && body.error)
...
Searcher.prototype.remove = function(document, callback)
{
	var self = this;
	var data = document.searchData();
	var indexpath = path.join(data.type, data.id);
	var url = this.urlbase + '/' + indexpath;

	bhttp.delete(url, function(err, response, body)
	{
		if (body && body.error) err = body.error;
		callback(err, body);
	});
...
Searcher.prototype.createTagMapping = function(callback)
{
	...
	bhttp.put(this.urlbase, function(err, response, data)
	{
		...
		bhttp.put(this.urlbase + '/' + mappath, function(err, response, body)
		{
			if (body && body.error) err = body.error;
			callback(err, body);
		});
...
Searcher.prototype.dropTagIndex = function(callback)
{
	...
	bhttp.delete(this.urlbase + '/tags', function(err, response, body)
	{
		if (body && body.error) err = body.error;
...
Searcher.prototype.indexTag = function(tag, callback)
{
	...
	var data = {type: 'tag', id: tag, tag: tag, text: text, category: prefix };
	var indexpath = path.join('tags', data.id);
	var opts = { encodeJson: true };
	var url = this.urlbase + '/' + indexpath;
...
Searcher.prototype.execute = function(options, callback)
{
	...
	bhttp.request(options, function(err, response, body)
	{
		var results = {
			tags: [],
			people: [],
