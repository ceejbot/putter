var
	...
	// StreamController = require('./streams'),
	util           = require('util'),
...
var Putter = module.exports = function Putter(config)
{
	...
	_.each(models, function(modelObj, item)
	{
		modelObj.setStorage(config.database, RethinkAdapter);
		modelObj.adapter.once('ready', function()
		{
			modelObj.adapter.provision(function() { self.logger.info(item + ' provisioned'); });
		});
		modelObj.adapter.connect();
	});

	this.searchdb = new search.Searcher(config.elasticsearch);
	// this.streams = new StreamController(config.subscriptions);
	this.invitations = new Invitations();
};

Putter.prototype.shutdown = function(callback)
{
	// TODO
	callback();
};
...
{
	models.Person.findByEmail_primary(email, function(err, people)
	{
		if (err) return callback(err, false);
		if (people.length === 0) return callback();
		callback(null, people[0]);
