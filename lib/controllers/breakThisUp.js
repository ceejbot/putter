/*
The app controller.
Manages operations upon documents: pulls all polyglot storage together.
Has a pointer to every model class persisted in CouchDB.
Has several catalogs to find things using redis indexes.
Has a search instance.
*/

var
    async                 = require('async'),
    catalogs              = require('./indexes'),
    completer             = require('prefix-completer'),
    CouchAdapter          = require('polyclay-couch'),
    cradle                = require('cradle'),
    models                = require('./models'),
    polyclay              = require('polyclay'),
    search                = require('./controllers/search'),
    sparkutils            = require('../utilities'),
    StreamController      = require('./controllers/streams'),
    InvitationsController = require('./controllers/invitations'),
    util                  = require('util')
	;

var Sparky = module.exports = function Sparky(configuration)
{
	// TODO still messier than I'd like
	var item, service, i, keys, name, modelObj;
	var stackConfig = configuration.stack;
	var logging = configuration.logging;

	this.name = stackConfig.name;
	this.logger = sparkutils.createLogger(this.name, logging);

	var couchConn = new cradle.Connection(
		configuration.couch.couch_host,
		configuration.couch.couch_port,
		{
			cache: false,
			raw: false,
			auth: configuration.couch_credentials
		}
	);

	var couchModels = Object.keys(models);

	for (i = 0; i < couchModels.length; i++)
	{
		item = couchModels[i];
		modelObj = models[item];
		modelObj.setStorage({ connection: couchConn, dbname: modelObj.prototype.plural}, CouchAdapter);
		this[modelObj.prototype.plural] = modelObj;
		this.logger.info(modelObj.prototype.plural + ' couchdb configured');
	}

	keys = Object.keys(stackConfig.catalogs);
	for (i = 0; i < keys.length; i++)
	{
		name = keys[i];
		item = stackConfig.catalogs[name];
		item.logging = logging;
		service = new catalogs[name]();
		service.configure(item);
		this[name] = service;
		this.logger.info(item.name + ' redis service configured');
	}

	this.searchdb = new search.Searcher(configuration.elasticsearch);
	this.completer = {};
	this.completer.tag = completer.create(configuration.completions);
	this.completer.fandom = completer.create(configuration.fandom_completions);
	this.completer.handles = completer.create(configuration.handle_completions);

	this.streams = new StreamController(configuration.subscriptions);
	this.invitations = new InvitationsController();
}

Sparky.prototype.shutdown = function(callback)
{
	// TODO
	callback();
};

Sparky.prototype.logAndBail = function(prefix, err, callback)
{
...
Sparky.prototype.register = function(person, inviteCode, callback)
{
	var self = this;
...
Sparky.prototype.savePerson = function(person, callback)
{
	var self = this;
	...
	var indexit = function(err, response)
	{
		...
		{
			self.completer.handles.add(person.handle, function(err, added)
			{
				callback(err, response); // not the es response or the redis response
			});
		});
	};
	...
	{
		if (!person.valid())
			return callback(person.errors);

		self.PeopleCatalog.generateKey(person.handle, function(err, response)
		{
			if (err) return callback(err, 'failure generating key');
			person.key = response;
			person.save(indexit);
		});
	}
...
Sparky.prototype.authenticate = function(handle, password, callback)
{
	var person,
		self = this;
	...
	self.PeopleCatalog.lookup(handle, function(err, key)
	{
		if (err || !key) return callback(err, false);
		self.people.get(key, function(err, person)
		{
...
Sparky.prototype.authenticateByEmail = function(email, password, callback)
{
	var self = this;
	self.people.findByEmail(email, function(err, person)
	{
		if (err || !person) return callback(err, false);
		person.authenticate(password, callback);
	});
};

Sparky.prototype.peopleByIDs = function(ids, callback)
{
	var results = {}, i, d, self=this;
	self.people.get(ids, function(err, results)
	{
		if (err) return self.logAndBail('peopleByIDs', err, callback);
		return callback(null, results);
	});
};

Sparky.prototype.personByID = function(id, callback)
{
	var person, self = this;
	if (!id) return callback('id parameter not supplied');
	self.people.get(id, callback);
};

Sparky.prototype.personByHandle = function(handle, callback)
{
	var person, self = this;
	if (!handle) return callback('handle parameter not supplied');
	self.PeopleCatalog.lookup(handle, function(err, id)
	{
		if (err) return self.logAndBail('lookup', err, callback);
		self.people.get(id, function(err, person)
		{
			if (err) return self.logAndBail('people.get', err, callback);
			callback(null, person);
		});
	});
};

Sparky.prototype.autocompleteHandle = function(handle, callback)
{
	var self = this;
...
Sparky.prototype.personByEmail = function(email, callback)
{
	var self = this;
	models.Person.findByEmail(email, function(err, person)
	{
		// any work to do?
		callback(err, person);
	});
...
Sparky.prototype.removePerson = function(person, callback)
{
...
Sparky.prototype.profile = function(person, callback)
{
	var self = this;
...
Sparky.prototype.profileFull = function(person, callback)
{
	var self = this;
...
Sparky.prototype.allFic = function(page, pageSize, callback)
{
...
Sparky.prototype.fetchFic = function(id, callback)
{
...
Sparky.prototype.ficByIDs = function(ids, callback)
{
	var self = this;
...
Sparky.prototype.fetchFicRendered = function(id, callback)
{
	var self = this;
	if (!id) return callback('id parameter not supplied');
...
Sparky.prototype.random = function(callback)
{
	this.FicCatalog.random(callback);
};

Sparky.prototype.randomRecent = function(callback)
{
	this.FicCatalog.randomRecent(callback);
};

Sparky.prototype.ficForPerson = function(input, page, pagesize, callback)
{
	var id, self = this;
...
Sparky.prototype.draftsForPerson = function(input, callback)
{
	var id, self = this;
	if (typeof input === 'object')
		id = input.key;
...
Sparky.prototype.storeSeries = function(series, callback)
{
	var self = this;
...
Sparky.prototype.generateFicID = function(fic, callback)
{
	var self = this;
	var id = sparkutils.randomID(6);
...
Sparky.prototype.storeFic = function(document, callback)
{
	var self = this;
...
Sparky.prototype.publishFic = function(document, callback)
{
	var self = this;
...
Sparky.prototype.unpublishFic = function(document, deleting, callback)
{
	var self = this;
...
Sparky.prototype.removeFic = function(document, callback)
{
	var self = this;
	var actions = [];
...
Sparky.prototype.tags = function(callback)
{
	var self = this;
...
Sparky.prototype.addTags = function(tagl, callback)
{
	var self = this;
...
Sparky.prototype.topNTags = function(count, callback)
{
	var self = this;
...
Sparky.prototype.topNTagsFiltered = function(count, filter, callback)
{
	var self = this;
	var result = {};
	var offset = 0;
...
Sparky.prototype.typeTags = function(callback)
{
...
Sparky.prototype.ficForTag = function(tag, page, pagesize, callback)
{
	var self = this;
...
Sparky.prototype.autocompleteTag = function(prefix, callback)
{
	var self = this;
...
Sparky.prototype.allFandoms = function(callback)
{
	var self = this;
...
Sparky.prototype.recentFandoms = function(callback)
{
	var self = this;
...
Sparky.prototype.fandomData = function(tag, callback)
{
	var self = this;
	var result = {};
...
Sparky.prototype.fandomPairings = function(tag, callback)
{
	this.FandomsCatalog.ficCountsByAllPairings(tag, callback);
};

Sparky.prototype.ficForFandom = function(ftag, page, pagesize, callback)
{
	var self = this;
...
Sparky.prototype.randomFicByFandom = function(ftag, callback)
{
	this.FandomsCatalog.randomFic(ftag, callback);
};

Sparky.prototype.ficForFandomCharacter = function(ftag, character, callback)
{
...
Sparky.prototype.ficForFandomPairing = function(ftag, pairing, callback)
{
	// TODO paginate eventually
	var self = this;
	var result = {};
...
Sparky.prototype.createFandom = function(fandom, callback)
{
	var self = this;
...
Sparky.prototype.removeFandom = function(fandom, callback)
{
	var self = this;
...
Sparky.prototype.autocompleteFandom = function(prefix, callback)
{
	var self = this;
...
Sparky.prototype.likesByFic = function(document, callback)
{
	var self = this;
...
Sparky.prototype.likesByPerson = function(person, callback)
{
	var self = this;
...
Sparky.prototype.like = function(person, document, callback)
{
	var self = this;
...
Sparky.prototype.unlike = function(person, document, callback)
{
	var self = this;
	if (person.key === document.owner_id)
	{
...
Sparky.prototype.feedback = function(document, callback)
{
	var self = this;
...
Sparky.prototype.feedbackSubtree = function(id, callback)
{
	var self = this;
...
Sparky.prototype.leaveFeedback = function(person, document, feedback, callback)
{
	var self = this;
...
Sparky.prototype.respondToComment = function(person, parent_id, feedback, callback)
{
	var self = this;
...
Sparky.prototype.feedbackByPerson = function(person, callback)
{
	models.Comment.findByOwner(person, callback);
};

Sparky.prototype.hideComment = function(actor, comment, callback)
{
...
Sparky.prototype.deleteComment = function(actor, comment, callback)
{
...
Sparky.prototype.review = function(person, document, data, callback)
{
	if (person.key === document.owner_id)
...
Sparky.prototype.reviewsByPerson = function(person, callback)
{
	models.Review.findByOwner(person.key, callback);
};

Sparky.prototype.reviewsByStory = function(document, callback)
{
	models.Review.findByTarget(document.key, callback);
};

Sparky.prototype.reviewByID = function(id, callback)
{
	if (!id || !id.length)
		return callback(null, []);
...
Sparky.prototype.fetchReview = function(person, target, callback)
{
...
Sparky.prototype.removeReview = function(review, callback)
{
	var self = this;
...
Sparky.prototype.recordVisit = function(person, document, callback)
{
	this.updateReadingLog(person, document, 'visit', null, callback);
};

Sparky.prototype.logEntry = function(person, document, createIfNeeded, callback)
{
	if (person.key === document.owner_id)
...
Sparky.prototype.updateReadingLog = function(person, document, action, target, callback)
{
...
Sparky.prototype.readingLog = function(person, page, pagesize, callback)
{
	var self = this;
	var result = {};
...
Sparky.prototype.hideLogEntry = function(person, entry, callback)
{
	var self = this;
...
Sparky.prototype.follow = function(person, target, callback)
{
	this.PeopleCatalog.follow(person, target, callback);
};

Sparky.prototype.unfollow = function(person, target, callback)
{
	this.PeopleCatalog.unfollow(person, target, callback);
};

Sparky.prototype.followers = function(person, callback)
{
	var self = this;
...
Sparky.prototype.followersByHandle = function(person, callback)
{
	var self = this;
	this.PeopleCatalog.followersByHandle(person, callback);
};

Sparky.prototype.following = function(person, callback)
{
	var self = this;
...
Sparky.prototype.followingByHandle = function(person, callback)
{
	var self = this;
...
Sparky.prototype.search = function(terms, callback)
{
	var self = this;
...
Sparky.prototype.serviceName = 'controller';
